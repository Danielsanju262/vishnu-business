...
function getRecurringPeriodStart(date: Date, recurrenceType: 'daily' | 'weekly' | 'monthly' | 'yearly'): Date {
...
function getRecurringPeriodEnd(date: Date, recurrenceType: 'daily' | 'weekly' | 'monthly' | 'yearly'): Date {
    switch (recurrenceType) {
        case 'daily': {
            return new Date(date.getFullYear(), date.getMonth(), date.getDate());
        }
...
formStartDate = format(startOfMonth(today), 'yyyy-MM-dd');
